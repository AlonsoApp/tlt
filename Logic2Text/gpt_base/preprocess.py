...
def text2id(subdir):
    ...
    for set in ["test"]: # ["all_data", "train", "valid", "test"]:
...
def build_values_extra(cased_values, value_cases_in_extra):
    values_extra = []
...
def values2id(subdir, value_cases_in_extra):
    '''
    convert text to bpe encoding
    '''
    ...
    for set in ["test"]:#["all_data", "train", "valid", "test"]:
        original = os.path.join(subdir, "original_data", set + ".json")
        processed_extra_values = os.path.join(subdir, "processed_data", set, set + "_extra_values.id")

        extra_values_out = open(processed_extra_values, "w")

        with open(original) as f:
            data = json.load(f)
            for tup in tqdm(data):

                logic_str = tup["logic_str"]
                cased_values = get_cased_values(logic_str)
                values_extra = build_values_extra(cased_values, value_cases_in_extra)
                values_extra_str = " ; ".join(values_extra)

                tokens_extra_values, _ = enc.encode(values_extra_str)

                extra_values_out.write(" ".join([str(token) for token in tokens_extra_values]) + "\n")

        extra_values_out.close()
...
def preprocess(subdir):
    ...
    text2id(subdir)
    values2id(subdir, "case1a;case1b;case2;case3")
...
def make_dirs(subdir):
    ...
    os.mkdir(os.path.join(subdir, "processed_data", "all_data"))
