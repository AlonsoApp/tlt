...
class Preprocessor:
    def __init__(self, data_dir, limits, eos, empty, bpe_enc, max_text_len, max_input_len):
        ...
        self.all_data_set = self.load_data('all_data') #None
        self.train_set = self.load_data('train') #None
        self.test_set = self.load_data('test')
        self.dev_set = self.load_data('valid') #None
...
class DataLoader:
    ...
    def get_zipped_batch(self, data, start_index, end_index):
        ...
        return zip(data['text'][start_index:end_index],
                   ...
                   data['extra_values'][start_index:end_index])
    ...
    def get_batch(self):
        ...
        max_extra_values_len = max([len(sample) for sample in self.data['extra_values'][start_index:end_index]])
        ...
        for text, topic, logic_interpret, logic, header, table, extra_values in data_subset:
            ...
            input += logic
            # uncomment this to use extra_values (Content Selection values)
            #input += extra_values
