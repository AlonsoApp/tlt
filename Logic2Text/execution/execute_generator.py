import sys
import os
import json
import csv
import random
from tqdm import tqdm
from collections import defaultdict
import numpy as np
import pandas as pd
from treelib import Node as NodePlt
from treelib import Tree as TreePlt
import traceback

#from generation.evaluation import EvalNode

from APIs import *

from generation.generator import aggregation, comparative, count, majority, ordinal, superlative, unique

actions_fn = {"aggregation":aggregation, "comparative":comparative, "count":count,
			  "majority":majority, "ordinal":ordinal, "superlative":superlative, "unique":unique}

class Node(object):
	def __init__(self, full_table, dict_in):
		'''
		construct tree
		'''

		self.full_table = full_table
		self.dict_in = dict_in
		self.func = dict_in["func"]

		# row, num, str, obj, header, bool
		self.arg_type_list = APIs[self.func]["argument"]
		self.arg_list = []

		# [("text_node", a), ("func_node", b)]
		self.child_list = []
		child_list = dict_in["args"]

		assert len(self.arg_type_list) == len(child_list)

		# bool, num, str, row
		self.out_type = APIs[self.func]["output"]

		for each_child in child_list:
			if isinstance(each_child, str):
				self.child_list.append(("text_node", each_child))
			elif isinstance(each_child, dict):
				sub_func_node = Node(self.full_table, each_child)
				self.child_list.append(("func_node", sub_func_node))
			else:
				raise ValueError("child type error")


		self.result = None

	def eval(self):

		for each_child, each_type in zip(self.child_list, self.arg_type_list):
			if each_child[0] == "text_node":
				if each_child[1] == "all_rows":
					self.arg_list.append(self.full_table)
				else:
					self.arg_list.append(each_child[1])
			else:
				sub_result = each_child[1].eval()
				# print ("exit func: ", each_child[1].func)

				# invalid
				if isinstance(sub_result, ExeError):
					print ("sublevel error")
					return ExeError()
				elif each_type == "row":
					if not isinstance(sub_result, pd.DataFrame):
						print ("error function return type")
						return ExeError()
				elif each_type == "bool":
					if not isinstance(sub_result, bool):
						print ("error function return type")
						return ExeError()
				elif each_type == "str":
					if not isinstance(sub_result, str):
						print ("error function return type")
						return ExeError()


				self.arg_list.append(sub_result)


		result = APIs[self.func]["function"](*self.arg_list)
		return result

	def show(self):
		tree = TreePlt()
		self._show_req(tree)
		print()
		tree.show()

	def _show_req(self, tree, parent=None):
		if parent is None:
			node = tree.create_node(self.func)
		else:
			node = tree.create_node(self.func, parent=parent)
		for name, value in self.child_list:
			if name == "text_node":
				tree.create_node(value, parent=node)
			elif name == "func_node":
				value._show_req(tree, parent=node)

def create_table(table_header, table_cont):
	pd_in = defaultdict(list)
	for ind, header in enumerate(table_header):
		for inr, row in enumerate(table_cont):
			# remove last summarization row
			if inr == len(table_cont) - 1 \
					and ("all" in row[0] or "total" in row[0] or "sum" in row[0] or \
						 "a l l" in row[0] or "t o t a l" in row[0] or "s u m" in row[0]):
				continue
			pd_in[header].append(row[ind])

	return pd.DataFrame(pd_in)

def execute_all(json_in):
	'''
	execute all logic forms
	'''
	with open(json_in) as f:
		data_in = json.load(f)

	json_out = []
	num_all = 0
	num_correct = 0
	num_error = 0

	for data in tqdm(data_in):
		action = data["action"]

		num_all += 1
		logic = data["logic"]

		table_header = data["table_header"]
		table_cont = data["table_cont"]

		try:
			pd_table = create_table(table_header, table_cont)
		except Exception:
			continue
		# ------
		try:
			random_logic_node = random.choice(list(actions_fn.values()))(pd_table)
			random_logic = random_logic_node.todic()
			root = Node(pd_table, random_logic)
		except Exception as ex:
			num_error += 1
			traceback.print_exc()
			continue
		# ------
		#root = Node(pd_table, logic)
		root.show()
		#eval_node = EvalNode(root)
		#tmp = eval_node.to_amr()
		#print("--------------------------------")
		res = root.eval()

		if res:
			num_correct += 1
			data["sent"] = "No text"
			data["annotation"] = None
			data["logic"] = random_logic
			data["logic_str"] = random_logic_node.tostr()
			data["interpret"] = "No interpret"
			json_out.append(data)

	print ("All: ", num_all)
	print ("Correct: ", num_correct)
	print ("Correctness Rate: ", float(num_correct) / num_all)
	return json_out

def save_out(json_out, mode):
	json_path = os.path.join(out_path, '{}.json'.format(mode))
	with open(json_path, 'w') as f:
		json.dump(json_out, f)
	# Create fake .text files
	text_path = os.path.join(out_path, '{}.text'.format(mode))
	with open(text_path, 'w') as f:
		f.write("No text\n" * (len(json_out) - 1))
		f.write("No text")

if __name__=='__main__':

	data_path = "../dataset"
	out_path = os.path.join(data_path, "original_data")
	#os.makedirs(out_path, exist_ok=False)
	json_all = []
	for mode in ["train", "valid", "test"]:
		json_in = os.path.join(data_path, "original_data", "{}.json".format(mode))
		json_out = execute_all(json_in)
		#save_out(json_out, mode)
		json_all = json_all + json_out
	# save all_data OJO! no está ordenado como el all_data original son train + valid + test pegados uno detras de otro
	#save_out(json_all, "all_data")