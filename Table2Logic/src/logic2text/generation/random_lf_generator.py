...
def generate_random_lf(pd_table: pd.DataFrame, action_fn: str = None, max_retry_iterations=20) -> str:
    """
    Generates a random LF based on a table.
    :param pd_table: the original table in pandas DataFrame format
    :param action_fn: if we want a random LF of a specific action type
    :param max_retry_iterations: sometimes the generator finds itself in situations where it can go no further and
    crashes (e.g. must choose a numeric column but the table has none). We retry the generation max_retry_iterations
    ...
    retry_count = 0
    retry = True
    while retry and retry_count <= max_retry_iterations:
        try:
            if action_fn is not None:
                random_logic_node = actions_fn[action_fn](pd_table)
            else:
                random_logic_node = random.choice(list(actions_fn.values()))(pd_table)
            return random_logic_node.todic()['tostr']
        except:
            retry_count += 1
            retry = True
            print("GEN: Retry {}".format(retry_count))
    raise Exception("Couldn't generate a random LF in {} attempts".format(retry_count))