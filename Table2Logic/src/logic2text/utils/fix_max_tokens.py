...
from logic2text.dataset_utils import split_into_tokens
from logic2text.utils import preprocess_table
...
def fix_dataset(original_path, fix_path, file_name):
    ...
    for sample in tqdm(dataset):
        ...
        values_extra = cased_values["case1a"] + cased_values["case1b"] + cased_values["case2"] + cased_values["case3"]  # values (case 1b, 2 and 3)

        input_ids_tensor, attention_mask_tensor, segment_ids_tensor, input_lengths = encode_input([topic],
                                                                                                  [table],
                                                                                                  [columns],
                                                                                                  [values_extra],
                                                                                                  tokenizer,
        ...
        #result = meets_max_size(topic, table, columns, values_extra, tokenizer, 512)
        if input_ids_tensor.size(1)<=512:
    ...
    print("Saving {} out of {} original values".format(len(fixed_dataset), len(dataset)))
    print("Removed {} samples".format(len(dataset) - len(fixed_dataset)))
...
def meets_max_size(topic, table, columns, values_extra, tokenizer, max_length_model):
    topic_tokens, topic_span_lengths, topic_segment_ids = _tokenize_topic(topic, tokenizer)
    ...
    values_extra_tokens, values_extra_token_lengths, values_extra_segment_ids = _tokenize_values_extra(values_extra,
                                                                                                       tokenizer)

    assert sum(topic_span_lengths) + sum(table_token_lengths) + sum(column_token_lengths) + \
           sum(values_extra_token_lengths) == len(topic_tokens) + len(table_tokens) + len(column_tokens) + len(
        values_extra_tokens)

    tokens = topic_tokens + table_tokens + column_tokens + values_extra_tokens
    return len(tokens) <= max_length_model
...
def run(original_path):
    fix_path = "./data/Logic2Text/original_data_fix_tokens/"
...
if __name__=='__main__':
    run("./data/Logic2Text/original_data_fix_values/")
