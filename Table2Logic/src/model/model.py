...
from model.example import Batch
...
class IRNet(BasicModel):
    ...
    def __init__(self, args, device, grammar):
        ...
        # embeddings that represent each possible I
        self.index_embed = nn.Embedding(args.max_index, args.index_embed_size, device=device)

        # the oov token tensor (token that refers to oov value cases), can be [0, embed_size] if we don't use any
        oov_tokens = 1 if args.include_oov_token else 0
        self.oov_token_tensor = nn.Embedding(oov_tokens, args.embed_size, device=device).weight.data

        self.att_project = nn.Linear(args.hidden_size + args.type_embed_size, args.hidden_size)

        self.N_embed = nn.Embedding(len(lf_grammar.N._init_grammar()), args.action_embed_size)
        ...
        self.index_rnn_input = nn.Linear(args.col_embed_size, args.action_embed_size, bias=False)
        ...
        self.index_pointer_net = PointerNet(args.hidden_size, args.col_embed_size, attention_type=args.column_att)
        ...
        nn.init.xavier_normal_(self.index_embed.weight.data)
        print('Use Column Pointer: ', True if self.use_column_pointer else False)

    def forward(self, examples):
        args = self.args
        # now should implement the examples
        # "grammar" is the lf_grammar language. It contains lookup tables (string to id <--> id to string)
        ...
        topic_encodings, table_encodings, column_encodings, values_encodings, transformer_pooling_output = self.encoder(batch.topics,
                                                                                                                       batch.tables,
                                                                                                                       batch.column_names,
                                                                                                                       batch.values_extra,
                                                                                                                       self.oov_token_tensor)
        # topics, tables, column_names, values
        # Concat topic and table encodings to form the input of the decoder. Equivalent of question_encodings on valuenet
        topic_table_encodings = torch.cat((topic_encodings, table_encodings), 1)
        #question_encodings = self.dropout(question_encodings)

        # Source encodings to create the sketch (the AST without the leaf-nodes)
        topic_table_encodings_sketch_linear = self.att_sketch_linear(topic_table_encodings)
        # Source encodings to create the leaf-nodes
        topic_table_encodings_lf_linear = self.att_lf_linear(topic_table_encodings)

        dec_init_vec = self.init_decoder_state(transformer_pooling_output)
        h_tm1 = dec_init_vec
        action_probs = [[] for _ in examples]

        zero_action_embed = Variable(self.new_tensor(args.action_embed_size).zero_())  # noop action embedding
        ...
        for t in range(batch.max_sketch_num):
            ...
            # All this information together form the input ("x") for the next step.
                ...
                for e_id, example in enumerate(examples):
                    ...
                    if t < len(example.sketch):
                        ...
                        if type(action_tm1) in self.grammar.sketch_actions():
            ...
            (h_t, cell_t), att_t, aw = self.step(x, h_tm1, topic_table_encodings,
                                                 topic_table_encodings_sketch_linear, self.sketch_decoder_lstm,
            ...
            # Ursin: What do we afterwards have in apply_rule_prob? We have a vector size (batch * 43) with
            # probabilities. 43 is the amount of possible actions (see self.grammar.prod2id).
        ...
        # Let's expand the embedding tensor to batch_size times to match [batch_size, max_index, index_embd_size]
        batch_index_embed = self.index_embed.weight.expand(len(examples), args.max_index, args.index_embed_size)
        ...
        # are in the end only interested in the actions creating a Leaf-Node (so C, I and V). We therefore also work with "example.tgt_actions" and not with "example.sketch" as above.
        ...
        for t in range(batch.max_action_num):
            if t == 0:
                ...
                for e_id, example in enumerate(examples):
                    if t < len(example.tgt_actions):
                        ...
                        if type(action_tm1) in self.grammar.terminal_actions():
                            # previous action C is a leaf-node, so we want to feed in the right Column-embedding. We select the column idx by using the ground truth "id_c".
                            if isinstance(action_tm1, lf_grammar.C):
                                a_tm1_embed = self.column_rnn_input(column_encodings[e_id, action_tm1.id_c])
                            # previous action I is a leaf-node, so we want to feed in the right Index-embedding. We select the table by using the ground truth "id_c".
                            elif isinstance(action_tm1, lf_grammar.I):
                                a_tm1_embed = self.index_rnn_input(batch_index_embed[e_id, action_tm1.id_c])
                            # previous action V is a leaf-node, so we want to feed in the right Value-embedding. We select the Value by using the ground truth "id_c".
                            elif isinstance(action_tm1, lf_grammar.V):
                                a_tm1_embed = self.value_rnn_input(values_encodings[e_id, action_tm1.id_c])
                            else:
            ...
            (h_t, cell_t), att_t, aw = self.step(x, h_tm1, topic_table_encodings,
                                                 topic_table_encodings_lf_linear, self.lf_decoder_lstm,
            ...
            weights = self.column_pointer_net(src_encodings=column_encodings, query_vec=att_t.unsqueeze(0), src_token_mask=batch.column_token_mask)
            ...
            weights.data.masked_fill_(batch.column_token_mask.bool(), -float('inf'))

            # Calculate the probabilities for the selected columns.
            column_attention_weights = F.softmax(weights, dim=-1)

            # We do the same again to select an index.
            index_weights = self.index_pointer_net(src_encodings=batch_index_embed, query_vec=att_t.unsqueeze(0),
                                                   src_token_mask=None)
            ...
            index_token_mask = batch.index_token_mask.expand_as(index_weights)
            index_weights.data.masked_fill_(index_token_mask.bool(), -float('inf'))

            # Calculate the probabilities for the selected columns.
            index_weights = F.softmax(index_weights, dim=-1)

            # Select a value with the pointer network
            value_weights = self.value_pointer_net(src_encodings=values_encodings, query_vec=att_t.unsqueeze(0),
                                                   src_token_mask=None)
            ...
            # The "masked_fill_" function fills every position with a "True"  with the given value (minus infinity).
            # So the remaining values (the M in the beginning) is the actual columns.
            value_weights.data.masked_fill_(batch.pointer_value_token_mask.bool(), -float('inf')) #  valuenet uses -99999 I don't know why

            # Calculate the probabilities for the selected values.
            value_weights = F.softmax(value_weights, dim=-1)

            # Now we calculate the loss, but only for the leaf actions (C, I and V).
            ...
            for e_id, example in enumerate(examples):
                if t < len(example.tgt_actions):
                    ...
                    if isinstance(action_t, lf_grammar.C):
                        ...
                        action_probs[e_id].append(act_prob_t_i)
                    elif isinstance(action_t, lf_grammar.I):
                        act_prob_t_i = index_weights[e_id, action_t.id_c]  # ground truth says we are looking for a T action, we get the probability of predicting the right table (which is the value of the table-softmax at index id_c)
                        action_probs[e_id].append(act_prob_t_i)
                    elif isinstance(action_t, lf_grammar.V):
        ...
        stack = [torch.stack(action_probs_i, dim=0).log().sum() for action_probs_i in action_probs]
        lf_prob_var = torch.stack(stack, dim=0)
    ...
    def parse(self, examples, beam_size=5):
        ...
        # We use our transformer encoder to encode question together with the schema (columns and tables). See "TransformerEncoder" for details
        topic_encodings, table_encodings, column_encodings, values_encodings, transformer_pooling_output = self.encoder(
                                                                                                                    batch.topics,
                                                                                                                    batch.tables,
                                                                                                                    batch.column_names,
                                                                                                                    batch.values_extra,
        ...
        # We know when a candidate is completed based on the grammar: if there is no more sketch action possible, we consider the sketch-candidate (or hypothesis) to be completed.
        ...
        # We either stop if we have done all beams or when we hit a maximum decoding steps (50 by default).
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            exp_src_enconding = topic_table_encodings.expand(hyp_num, topic_table_encodings.size(1), topic_table_encodings.size(2))
            exp_src_encodings_sketch_linear = topic_table_encodings_sketch_linear.expand(hyp_num, topic_table_encodings_sketch_linear.size(1),
                                                                                       topic_table_encodings_sketch_linear.size(2))
            if t == 0:
                with torch.no_grad():
            ...
            for hyp_id, hyp in enumerate(beams):
                ...
                if action_class in self.grammar.sketch_actions():
        ...
        # Let's expand the embedding tensor to batch_size times to match [batch_size, max_index, index_embd_size]
        #batch_index_embed = self.index_embed.weight.expand(1, self.args.max_index, self.args.index_embed_size)

        ####################### PART 3: Select columns and tables (and A) ###########################################
        # Based on the Sketch of Part 1 we now know how the AST looks. We even added "dummy" actions for C, I and V. But what is still missing,
        # is selecting the right index to the columns, indexes and values. This is what we do in Part 3.
        # We use the best sketch from Part 1 and run a new RNN to predict columns/values.
        ...
        while len(completed_beams) < beam_size and t < self.args.decode_max_time_step:
            ...
            exp_src_encodings = topic_table_encodings.expand(hyp_num, topic_table_encodings.size(1), topic_table_encodings.size(2))
            exp_utterance_encodings_lf_linear = topic_table_encodings_lf_linear.expand(hyp_num, topic_table_encodings_lf_linear.size(1),
                                                                                     topic_table_encodings_lf_linear.size(2))
            exp_column_embedding = column_encodings.expand(hyp_num, column_encodings.size(1), column_encodings.size(2))

            exp_index_embedding = self.index_embed.weight.expand(hyp_num, self.args.max_index, self.args.index_embed_size)
            ...
            if t == 0:
                ...
                for e_id, hyp in enumerate(beams):
                    ...
                    if type(action_tm1) in self.grammar.sketch_actions():
                        a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[action_tm1.production]]
                        hyp.sketch_step += 1
                    elif isinstance(action_tm1, lf_grammar.C):
                        a_tm1_embed = self.column_rnn_input(column_encodings[0, action_tm1.id_c])    # the id_c is to select the right column-embedding, based on the column index we selected in the last step. The 0 is only necessary because the table_embedding has 2 dimensions (first is batch, which we don't use in inference).
                    elif isinstance(action_tm1, lf_grammar.I):
                        a_tm1_embed = self.index_rnn_input(exp_index_embedding[0, action_tm1.id_c])
                    elif isinstance(action_tm1, lf_grammar.V):
                        a_tm1_embed = self.value_rnn_input(values_encodings[0, action_tm1.id_c])
                    else:
                        raise ValueError('unknown action %s' % action_tm1)
            ...
            weights = self.column_pointer_net(src_encodings=exp_column_embedding, query_vec=att_t.unsqueeze(0),
                                              src_token_mask=batch.column_token_mask)

            # should this mask not get activated? In my understanding it is only here because different examples in the batch have different lengths
            # ------------> maybe not necessary because we anyway one have one beam - so only one example.
            ...
            index_weights = self.index_pointer_net(src_encodings=exp_index_embedding, query_vec=att_t.unsqueeze(0),
                                                   src_token_mask=None)
            # index_weights = self.table_pointer_net(src_encodings=exp_schema_embedding, query_vec=att_t.unsqueeze(0), src_token_mask=None)

            index_token_mask = batch.index_token_mask.expand_as(index_weights)
            index_weights.data.masked_fill_(index_token_mask.bool(), -float('inf'))

            # the probabilities we use in case of T action
            index_weights = F.log_softmax(index_weights, dim=-1)
            ...
            value_weights.data.masked_fill_(batch.pointer_value_token_mask.bool(), -float('inf'))
            ...
            for hyp_id, hyp in enumerate(beams):
                ...
                if type(padding_sketch[t]) == lf_grammar.C:
                    ...
                    for col_id, _ in enumerate(batch.column_names[0]):
                        ...
                        meta_entry = {'action_type': lf_grammar.C, 'col_id': col_id,
                                      'score': col_sel_score, 'new_hyp_score': new_hyp_score,
                                      'prev_hyp_id': hyp_id}
                        new_hyp_meta.append(meta_entry)
                elif type(padding_sketch[t]) == lf_grammar.I:
                    # similar to the C action
                    for i_id in range(self.args.max_index):
                        t_sel_score = index_weights[hyp_id, i_id]
                        new_hyp_score = hyp.score + t_sel_score.data.cpu()

                        meta_entry = {'action_type': lf_grammar.I, 'i_id': i_id,
                                      'score': t_sel_score, 'new_hyp_score': new_hyp_score,
                                      'prev_hyp_id': hyp_id}
                        new_hyp_meta.append(meta_entry)
                elif type(padding_sketch[t]) == lf_grammar.V:
                    # similar to the C and I action
                    for value_idx in range(batch.pointer_values_len[0]):
                        val_sel_score = value_weights[hyp_id, value_idx]
                        new_hyp_score = hyp.score + val_sel_score.data.cpu()

                        meta_entry = {'action_type': lf_grammar.V, 'val_id': value_idx,
                    ...
                    new_hyp_score = hyp.score + torch.tensor(0.0)   # by using a score of 0 for this case we make sure this does not change our scoring scheme: we want to make sure the score is only relating C, I and V in this part of the code! Remember, the sketch is already set above!
            ...
            for new_hyp_score, meta_id in zip(top_new_hyp_scores.data.cpu(), meta_ids.data.cpu()):
                ...
                # a prod_id exist only if it is a sketch action
                if 'prod_id' in hyp_meta_entry:
                    prod_id = hyp_meta_entry['prod_id']

                if action_type_str == lf_grammar.C:
                    col_id = hyp_meta_entry['col_id']
                    action = lf_grammar.C(col_id)    # remember, in action C the id_c is the index of the column.
                elif action_type_str == lf_grammar.I:
                    t_id = hyp_meta_entry['i_id']
                    action = lf_grammar.I(t_id)  # and for action I, it is the index of the table.
                elif action_type_str == lf_grammar.V:
                    val_id = hyp_meta_entry['val_id']
                    action = lf_grammar.V(val_id)  # and for action V, it is the index of the Value.
                # This is the case for sketch actions.
                ...
                # The way a hypothesis is completed doesn't really change compared to Part 1. As soon as there is no more Action class available, the hypothesis is completed.
                # what changes is the is_sketch-flag: the hypothesis will now only be done when also non-sketch actions (C, I and V) are set.
                if new_hyp.completed:
                    completed_beams.append(new_hyp)
                else:
                    new_beams.append(new_hyp)
