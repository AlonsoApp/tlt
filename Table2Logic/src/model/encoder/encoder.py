...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, pretrained_model, device, max_sequence_length, schema_embedding_size, decoder_hidden_size):
        ...
        # We don't want to do basic tokenizing (so splitting up a sentence into tokens) as this is already done in pre-processing.
        # But we still want to do the wordpiece-tokenizing.
    ...
    def forward(self, topics, tables, column_names, values_extra, oov_token_tensor):
        input_ids_tensor, attention_mask_tensor, segment_ids_tensor, input_lengths = encode_input(topics,
                                                                                                  tables,
                                                                                                  column_names,
                                                                                                  values_extra,
                                                                                                  self.tokenizer,
        ...
        last_hidden_states, pooling_output = self.transformer_model(input_ids_tensor, attention_mask_tensor, segment_ids_tensor, return_dict=False)

        (all_topic_span_lengths, all_table_token_lengths, all_column_token_lengths, all_values_extra_token_lengths) = input_lengths

        # we get the relevant hidden states for the topic-tokens and average, if there are multiple token per  word (e.g ['table', 'college'])
        averaged_hidden_states_topic, pointers_after_topic = self._average_hidden_states_topic(last_hidden_states, all_topic_span_lengths)
        topic_out = pad_sequence(averaged_hidden_states_topic, batch_first=True)  # (batch_size * max_topic_tokens_per_batch * hidden_size)
        ...
        topic_out = self.linear_layer_dimension_reduction_question(topic_out)

        table_hidden_states, pointers_after_table = self._get_schema_hidden_states(last_hidden_states, all_table_token_lengths, pointers_after_topic)
        column_hidden_states, pointers_after_columns = self._get_schema_hidden_states(last_hidden_states, all_column_token_lengths, pointers_after_table)

        # in this scenario, we know the values upfront and encode them similar to tables/columns. This is different as soon as we don't know the values but have to extract them from the table.
        values_extra_hidden_states, pointers_after_values_extra = self._get_schema_hidden_states(last_hidden_states, all_values_extra_token_lengths, pointers_after_columns)

        # This is simply to make sure the rather complex token-concatenation happens correctly. Can get removed at some point.
        self._assert_all_elements_processed(all_topic_span_lengths,
                                            all_table_token_lengths,
                                            all_column_token_lengths,
                                            all_values_extra_token_lengths,
                                            pointers_after_values_extra,
        ...
        # With this line we first concat all examples to one huge list of tensors, independent of the example. Remember: we don't want to use an RNN over a full example - but only over the tokens of ONE column! Therefore we can just build up a batch of each column - tensor.
        ...
        # create one embedding for each table cell by using an RNN.
        ...
        if list(flatten(values_extra_hidden_states)):
            value_hidden_states_padded = pad_sequence(list(flatten(values_extra_hidden_states)), batch_first=True)
            value_lengths = [len(t) for t in flatten(values_extra_hidden_states)]

            # create one embedding for each value by using an RNN.
            _, values_extra_last_states, _ = self._rnn_wrapper(self.value_encoder, value_hidden_states_padded, value_lengths)

            assert values_extra_last_states.shape[0] == sum(map(lambda l: len(l), values_extra_hidden_states))

            values_extra_out = self._back_to_original_size(values_extra_last_states, values_extra_hidden_states)
            # concat table_out + values_extra_out to form non intermediately padded values_out
            values_out = [torch.cat((table, values_extra_out[i], oov_token_tensor))for i, table in enumerate(table_out)]
            values_out_padded = pad_sequence(values_out, batch_first=True)
            self._assert_values_out(values_out, tables, values_extra, oov_token_tensor)
        else:
            values_out = [torch.cat((table, oov_token_tensor)) for i, table in enumerate(table_out)]
            values_out_padded = pad_sequence(values_out, batch_first=True)

        return topic_out, table_out_padded, column_out_padded, values_out_padded, pooling_output

    @staticmethod
    def _average_hidden_states_topic(last_hidden_states, all_topic_span_lengths):
        """
        As described in the IRNet-paper, we will just average over the sub-tokens of a topic-span.
        """
        all_averaged_hidden_states = []
        last_pointers = []

        for batch_itr_idx, topic_span_lengths in enumerate(all_topic_span_lengths):
            ...
            for idx in range(1, len(topic_span_lengths) - 1):
                span_length = topic_span_lengths[idx]
    ...
    def _get_schema_hidden_states(last_hidden_states, all_schema_token_lengths, initial_pointers):
        ...
        for batch_itr_idx, (schema_token_lengths, initial_pointer) in enumerate(zip(all_schema_token_lengths, initial_pointers)):
            ...
            for schema_token_length in schema_token_lengths:
                # the -1 represents the ':', ';' or [SEP] by the end of the value, column, which we don't want to include.
    ...
    @staticmethod
    def _assert_values_out(values_out, tables, values_extra, oov_token_tensor):
        for values, table, values_extra in zip(values_out, tables, values_extra):
            table_len = len([item for sublist in table for item in sublist])
            assert values.size(0) == (table_len + len(values_extra) + oov_token_tensor.size(0))